...
(async function () {
    ...
    const rows = [[], []]; // row 0 = above, row 1 = below
    const MIN_SPACING = 120; // pixels (adjust this)

    events.forEach((ev, i) => {
        const x = timeScale(new Date(ev.date));
        let row = 0;

        // find a row with no overlap
        while (true) {
            if (!rows[row]) rows[row] = [];

            const conflicts = rows[row].some(prevX => Math.abs(prevX - x) < MIN_SPACING);
        ...
        // positions
        const labelY = above
            ? LINE_Y - 18 - level * LAYER_HEIGHT
            : LINE_Y + 32 + level * LAYER_HEIGHT;
        ...
        const lineY2 = above
            ? LINE_Y - 14 - level * LAYER_HEIGHT
            : LINE_Y + 22 + level * LAYER_HEIGHT;
